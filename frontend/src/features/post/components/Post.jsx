...
const Post = ({ user, post, loading, handleLike, handleUnLike }) => {
    ...
    const [followStatus, setFollowStatus] = useState(null) // null = not clicked yet
    ...
    const onFollowClick = async () => {
        const data = await handleFollow(user.username)
        // data.follow.status will be "pending" or "accepted"
        if (data?.follow?.status) {
            setFollowStatus(data.follow.status)
        } else {
            setFollowStatus("pending") // default to pending
        }
    }
    ...
    return (
        <div className="post">
            <div className="user">
                ...
                {!isOwnPost && (
                    <button
                        className={`follow-btn ${followStatus ? 'follow-btn--active' : ''}`}
